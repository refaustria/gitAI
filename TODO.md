...
| **A CLI flag that is parsed and then ignored** — `--checkpoint-every`, `--keep-last` and `--resume` were accepted by `scripts/train.py` for its whole life while the loop that used them lived in the library; `--resume` trained from scratch and printed a normal result | Assert the *artifact* (a `step-*` directory exists, the resumed run matches bit-for-bit), never the exit code. Fail loudly when asked to resume with nothing to resume from |
| **Scripts had no tests at all** — both this and the `run_loop` seeding bug lived in callers while the libraries they called were correct and fully covered | `tests/test_scripts_cli.py` runs the entry points in a subprocess, because in-process `main()` calls would have caught neither |
| **A benchmark that times something other than what you train** — `scripts/benchmark.py` measured a Phase-1 proxy architecture and every scoping number in the project derived from it, overstating cost by 1.6–5× | Benchmark the real model. The docstring admitted the proxy was temporary; a known-temporary shortcut needs an issue, not a comment |
